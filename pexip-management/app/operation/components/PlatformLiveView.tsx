...
export default function PlatformLiveView() {
  ...
  const pieData = {
    ...
    datasets: [
      {
        data: [
          workers.filter((worker) => worker.node_type === 'CONFERENCING').length,
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300 p-6 w-full h-full">
        ...
        <ul className="space-y-4">
          {workers.map((worker) => (
            ...
            >
              ...
              <p><strong>Type:</strong> {worker.node_type}</p>
            </li>
          ))}
        </ul>
      </div>
      ...
      <div id="network-diagram" className="relative border bg-white">
        <h2 className="absolute top-0 left-0 p-2 bg-white">Network Diagram</h2>
      </div>

      {/* Biểu đồ Pie */}
      <div>
        <h3 className="text-lg font-semibold mb-4 text-gray-800">Worker Node Types</h3>
